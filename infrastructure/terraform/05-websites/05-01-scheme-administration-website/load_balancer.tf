resource "google_compute_global_address" "service_load_balancer_ip" {
  name = "${local.name_prefix}-scheme-administration-portal-lb-ip"
...
resource "google_compute_region_network_endpoint_group" "search_service_serverless_neg" {
  name                  = "${local.name_prefix}-scheme-administration-portal-serverless-neg"
...
module "loadbalancer" {
  ...
  version = "10.0.0"
  name    = "${local.name_prefix}-scheme-administration-portal-lb"
  project = var.project
  ...
  backends = {
    default = {
      description = "Scheme Administration Portal"
