...
public sealed class ProcessRepository : IProcessRepository
{
    ...
    public ProcessRepository(AppDbContext context) => _context = context;

    public async Task<Process?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
        => await _context.Processes.FirstOrDefaultAsync(x => x.Id == id, cancellationToken);

    public async Task<IEnumerable<Process>> GetPagedAsync(int page, int pageSize,
        CancellationToken cancellationToken = default)
        => await _context.Processes.OrderByDescending(x => x.LastUpdated).Skip(page * pageSize).Take(pageSize).ToListAsync(cancellationToken);
